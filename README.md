...
A purely local LLM-powered system for analyzing sales call transcripts and answering questions through an intelligent chat interface. Uses Ollama for local inference with structured data extraction and mathematical analysis to provide accurate, contextual responses without any external APIs.
...
Step 2: Local Ollama Installation
...
Model Switching & Result Variations


Configuring Different Models


Edit config.py to experiment with models:
...
1. Speaker Pattern Analysis (_analyze_speakers function)


Problem: "Who was the main participant?" requires scanning entire transcript
...
# Provides precise participation metrics
return {
...
The function searches for speaker patterns using regex like r'^([A-Z][a-z]+)\s*(\d+%)' for Otter.ai format or r'([A-Z][a-z]+)(?=:|\s+(?:says?|mentions?))' for general patterns. It counts words per speaker and calculates ratios where 0.58 means Shashwat spoke 58% of total words.


2. Content Density Analysis (_analyze_content_density function)
...
# Scans for technical terms, financial mentions, questions
# Calculates density ratios to identify content-rich segments
return {
...
Technical patterns include API|ML|AI|neural|transformer|RAG|vector|algorithm and backend|frontend|database|deployment|architecture. Financial patterns capture dollar amounts like r'$[\d,]+' and terms like budget|cost|price|revenue|profit|ROI|funding. Question density is calculated as len(re.findall(r'?', text)) / len(sentences) where higher values indicate more interactive segments.


3. Engagement Peak Detection (_analyze_conversation_flow function)


Problem: Where are the most important discussion moments?
...
For each line, length_score = min(len(line.split()) / 20, 1.0) normalizes to 0-1 range. question_score = 0.3 if '?' in line adds interaction weight. tech_score = min(tech_terms * 0.1, 0.5) weights technical complexity. financial_score = min(financial_terms * 0.15, 0.3) prioritizes business discussions. Total engagement = length_score + question_score + tech_score + financial_score capped at 2.0.


4. Topic Importance Weighting (_calculate_topic_importance function)


Problem: All extracted topics aren't equally important for queries
Solution: Mathematical importance scoring


def _calculate_topic_importance(self, semantic_data):
# Weights based on frequency, diversity, content length
# Guides context selection during query processing
return {
"feedback_given": 0.85,       # High weight - always include
"technical_topics": 0.67,     # Medium weight - include if space
"casual_chat": 0.12          # Low weight - exclude when tight
}


frequency_weight = len(items) / 10 normalizes item count to 0-1 range. diversity_weight = unique_items / total_items measures content variety. length_weight = min(avg_length / 5, 1.0) weights longer descriptions higher. combined_weight = (frequency_weight * 0.4 + diversity_weight * 0.3 + length_weight * 0.3) creates final importance score where 0.85 means high priority, 0.67 medium, 0.12 low.


Why Mathematical Metadata Enhances Sales Use Cases


Context Selection Intelligence: When user asks "What feedback was given?", the system uses importance weights to prioritize feedback-related content over casual conversation.
...
Engagement-Driven Context: Peak detection ensures LLM receives context from the most meaningful conversation moments, not random segments.
...
Strategy: Load complete semantic + mathematical context for target meeting
Context Fed to LLM: Full meeting data with mathematical patterns for enhanced understanding
...
Context Fed to LLM: Consolidated feedback summaries with mathematical validation
...
# feedback_focused: "feedback", "performance", "evaluation"
# meeting_specific: "meeting 1", "call 2", specific identifiers
# entity_focused: "budget", "technical", "objections"
# aggregate: "all", "common", "overall", "summary"


The function uses keyword matching where feedback_keywords = ['feedback', 'performance', 'told', 'said about', 'advice', 'improve', 'criticism', 'evaluation'] triggers feedback_focused classification. Pattern re.search(r'\b(?:meeting|call|interview)\s+\d+\b', query_lower) detects meeting_specific queries. entity_keywords = ['budget', 'financial', 'objection', 'technical', 'concern'] triggers entity_focused. Each classification routes to optimized context selection.
...
Mathematical analysis shows 127 speaking turns for Shashwat and 89 for Rohit, indicating interactive dialogue format typical of technical interviews.


2. Feedback Extraction


Query: "What specific feedback was given to Shashwat?"


Output:
Feedback summary for Shashwat:
...
The mathematical importance weighting identified this feedback as high-priority content (0.85 importance score).


3. Technical Content Analysis


Query: "What technical topics were discussed?"
...
Mathematical clustering identified 3 distinct technical topic groups with transformer architectures having highest importance score.


4. Assignment Details


Query: "What assignment was given?"


Output:
Assignment extracted from high-engagement conversation segment:
...
Speaker analysis confirms assignment was given during peak engagement period (conversation energy: 2.4/3.0).


5. Cross-Meeting Summary


Query: "Summarize all meetings comprehensively"


Output:
Comprehensive analysis across all processed meetings:
...
Business impact: Candidate assessment with actionable development path
...
Mathematical Intelligence: Quantitative analysis guides optimal context selection
Model Flexibility: Easy switching between Ollama models for different needs
